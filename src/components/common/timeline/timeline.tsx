  ...
  ({ className, iconsize, size, children, ...props }, ref) => {
    ...
    return (
      <ol
        ref={ref}
        aria-label="Timeline"
        className={cn(timelineVariants({ size }), "relative min-h-[600px] w-full py-8", className)}
        {...props}
      >
...
const TimelineItem = React.forwardRef<HTMLLIElement, TimelineItemProps>(
  (
    ...
    const content = (
      <div
        className="flex md:gap-8 gap-6 items-start"
      ...
      >
        <div className="flex flex-col items-center w-10">
          ...
          {showConnector && <div className="h-32 w-0.5 bg-border mt-2" />}
        </div>

        {/* Content */}
        <TimelineContent>
          <TimelineHeader className="flex flex-col items-start gap-0">
            {/* Date */}
            <TimelineTime className="">{date}</TimelineTime>
  ...
  ({ className, date, format, children, ...props }, ref) => {
    ...
    return (
      <time
        ...
        className={cn("md:text-xl text-lg font-semibold", className)}
        {...props}
      >
...
const TimelineTitle = React.forwardRef<HTMLHeadingElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, children, ...props }, ref) => (
    <h3
      ref={ref}
      className={cn("font-semibold md:leading-none md:text-lg leading-5 tracking-tight text-ip-primary-500", className)}
      {...props}
    >
      {children}
    </h3>
...
const TimelineIcon = ({
  ...
  const colorClasses = {
    primary: "md:bg-white bg-ip-gray-200 text-primary-foreground",
...
const TimelineDescription = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(
  ({ className, ...props }, ref) => (
    <p ref={ref} className={cn("md:text-sm text-xs text-ip-gray-600", className)} {...props} />
  )
);
...
const TimelineContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div ref={ref} className="relative w-full">
      <div className={cn("flex flex-col gap-3 md:bg-white bg-ip-gray-200 p-5", className)} {...props} />
      <Image
        src="/assets/icons/ic_arrow-left.svg"
        alt="ic_arrow-left.svg"
        width={10.98}
        height={23.15}
        className="absolute top-2.5 md:-left-3 -left-2"
      />
    </div>
  )
);
